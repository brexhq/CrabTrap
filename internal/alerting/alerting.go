...
func (s *Service) tryFlush() {
	...
	// Atomically claim and delete flushable denials. FOR UPDATE SKIP LOCKED
	// ensures only one replica processes each row — pgbouncer-compatible.
...
func (s *Service) flushBot(ctx context.Context, botID string, denials []DenialInfo) {
	...
	if len(managerIDs) == 0 {
		slog.Debug("alerting: no managers for bot", "bot_id", botID, "count", len(denials))
		return
	}
	...
	if len(channels) == 0 {
		slog.Debug("alerting: no channels for bot", "bot_id", botID, "count", len(denials))
		return
	}
